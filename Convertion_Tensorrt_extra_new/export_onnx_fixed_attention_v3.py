...
This version fixes the internal model patch size to match the actual DINOv2 weights.
"""
...
def load_weights_with_detailed_feedback(model, checkpoint_path: str) -> Tuple[bool, Dict]:
    ...
    try:
        ...
        for ckpt_key, value in state_dict.items():
            ...
            # Try some common mappings for MatchAnything checkpoint structure
            mapped_key = None
            
            # Remove common prefixes
            if ckpt_key.startswith('matcher.model.encoder.'):
                # matcher.model.encoder.cnn.layers.X -> encoder.layers.X
                mapped_key = ckpt_key.replace('matcher.model.encoder.', 'encoder.')
            elif ckpt_key.startswith('matcher.model.'):
        ...
        # Show some missing parameters for debugging (limit output)
        if missing:
            missing_sample = missing[:3] if len(missing) > 3 else missing
            print(f"[WEIGHTS] Some missing parameters: {missing_sample}")
        
        # Show some unexpected parameters for debugging (limit output)
        if unexpected:
            unexpected_sample = unexpected[:3] if len(unexpected) > 3 else unexpected
            print(f"[WEIGHTS] Some unexpected parameters: {unexpected_sample}")
        
        if stats['load_percentage'] >= 10:  # Very low threshold - any loading is better than none
            print("[WEIGHTS] ✅ Weights loaded (proceeding with partial loading)")
            return True, stats
        else:
            print("[WEIGHTS] ⚠️ Very low loading percentage, but proceeding anyway")
...
def fix_model_patch_size(model) -> int:
    """
    Fix the model's internal patch size to match the actual DINOv2 weights.
    Returns the corrected patch size.
    """
    try:
        # Get the actual patch size from DINOv2 component
        actual_patch_size = 16  # DINOv2 uses 16x16 patches
        ...
        if hasattr(model.encoder, 'dino') and hasattr(model.encoder.dino, 'patch_embed'):
            ...
            if hasattr(patch_embed, 'patch_size'):
                if isinstance(patch_embed.patch_size, (tuple, list)):
                    actual_patch_size = patch_embed.patch_size[0]
                else:
                    actual_patch_size = patch_embed.patch_size
            elif hasattr(patch_embed, 'proj'):
                # Infer from conv kernel size
                kernel_size = patch_embed.proj.kernel_size
                if isinstance(kernel_size, (tuple, list)):
                    actual_patch_size = kernel_size[0]
                else:
                    actual_patch_size = kernel_size
        
        # Fix the encoder's patch attribute
        if hasattr(model.encoder, 'patch'):
            old_patch = model.encoder.patch
            model.encoder.patch = actual_patch_size
            print(f"[PATCH] Fixed encoder patch size: {old_patch} → {actual_patch_size}")
        
        # Also fix it at the model level if it exists
        if hasattr(model, 'patch'):
            model.patch = actual_patch_size
            
        return actual_patch_size
        
    except Exception as e:
        print(f"[PATCH] Error fixing patch size: {e}")
...
def export_onnx_with_detailed_feedback(
    ...
    Fixes the internal patch size mismatch issue.
    """
    device = "cpu"
    ...
    # Fix the patch size mismatch BEFORE loading weights
    actual_patch_size = fix_model_patch_size(model)
    print(f"[EXPORT] Using patch size: {actual_patch_size}")
    ...
    with torch.inference_mode():
        try:
            ...
            print(f"[EXPORT] This might be due to remaining patch size issues or weight loading problems")
            import traceback
            traceback.print_exc()
            raise
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Export MatchAnything to ONNX with detailed feedback (v3 - patch fix)")
    parser.add_argument("--onnx", default="out/test_fixed_v3.onnx", 
