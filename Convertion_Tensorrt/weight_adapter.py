...
def _suffix_map(remapped: Dict[str, torch.Tensor], msd: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
...
def _print_diagnostics(state: Dict[str, torch.Tensor], msd: Dict[str, torch.Tensor], sample_n: int = 25):
    ...
    for p, c in top_prefixes(mk, depth=2):
        print(f"    {p:40s} x{c}")
    print(f"[WEIGHTS][DIAG] Sample CKPT keys:")
    for k in ck[:sample_n]:
        print(f"    - {k}")

def remap_and_load(model: nn.Module, ckpt_path: str, save_sanitized: Optional[str] = None) -> Dict[str, torch.Tensor]:
